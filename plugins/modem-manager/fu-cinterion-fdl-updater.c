/*
 * Copyright (C) 2024 TDT AG
 *                    Lukas Voegl <lvoegl@tdt.de>
 *
...
typedef enum {
	FDL_RESPONSE_OK = 0x01,
	FDL_RESPONSE_RETRY = 0x02,
	FDL_RESPONSE_UNKNOWN = 0x03,
	FDL_RESPONSE_BUSY = 0x04
} FuCinterionFdlResponse;
...
fu_cinterion_fdl_updater_wait_ready(FuCinterionFdlUpdater *self, FuDevice *device, GError **error)
{
	...
	for (guint i = 0; i < FU_CINTERION_FDL_MAX_READ_RETRIES; i++) {
		...
		if (bytes_read == 1 && byte == FDL_RESPONSE_OK) {
			return TRUE;
		}
...
fu_cinterion_fdl_updater_set_io_flags(FuCinterionFdlUpdater *self, GError **error)
{
	...
	if (tcflush(fd, TCIOFLUSH) != 0) {
		g_set_error_literal(error,
				    FWUPD_ERROR,
				    FWUPD_ERROR_NOT_SUPPORTED,
				    "could not flush termios attributes");
		return FALSE;
	}
			       ...
			       GError **error)
{
	...
	for (guint i = 0; i < FU_CINTERION_FDL_MAX_READ_RETRIES; i++) {
		...
		switch (byte) {
		case FDL_RESPONSE_OK:
			*response = FDL_RESPONSE_OK;
			break;
		case FDL_RESPONSE_RETRY:
			*response = FDL_RESPONSE_RETRY;
			break;
		case FDL_RESPONSE_BUSY:
			*response = FDL_RESPONSE_BUSY;
			break;
		default:
			*response = FDL_RESPONSE_UNKNOWN;
			break;
		}
					   ...
					   GError **error)
{
	...
	while (write_retries < FU_CINTERION_FDL_MAX_WRITE_RETRIES) {
		...
		while (read_retries < FU_CINTERION_FDL_MAX_READ_RETRIES) {
			...
			if (response == FDL_RESPONSE_OK) {
				/* chunk written successfully, stop reading */
				return TRUE;
			} else if (response == FDL_RESPONSE_BUSY) {
				/* retry reading response */
				read_retries++;
			} else if (response == FDL_RESPONSE_RETRY) {
			       ...
			       GError **error)
{
	...
	while (offset < fw_len) {
		...
		guint16 *chunk_size = NULL;
		guint16 chunk_size_le = 0;

		size_bytes = g_bytes_new_from_bytes(fw, offset, FU_CINTERION_FDL_SIZE_BYTES);
		chunk_size = (guint16 *)g_bytes_get_data(size_bytes, NULL);
		chunk_size_le = GUINT16_TO_LE(*chunk_size);
		offset += FU_CINTERION_FDL_SIZE_BYTES;

		chunk_bytes = g_bytes_new_from_bytes(fw, offset, chunk_size_le);
		offset += chunk_size_le;
