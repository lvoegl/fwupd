...
fu_mm_qcdm_device_decode_data(GBytes *res, GError **error)
{
	...
	return g_byte_array_free_to_bytes(g_steal_pointer(&res_decoded_array));
}
...
static gboolean
fu_mm_qcdm_device_switch_to_edl(FuDevice *device, GError **error)
{
	FuMmQcdmDevice *self = FU_MM_QCDM_DEVICE(device);
	g_autoptr(FuMmQcdmDevicePktRebootEdlReqRes) pkt_req = NULL;
	g_autoptr(FuMmQcdmDevicePktRebootEdlReqRes) pkt_res = NULL;
	g_autoptr(FuMmQcdmDeviceSubsystemReq) payload = NULL;
	...
	payload = fu_mm_qcdm_device_subsystem_req_new();
	fu_mm_qcdm_device_subsystem_req_set_command(payload, FU_MM_QCDM_DEVICE_COMMAND_COMMAND);
	fu_mm_qcdm_device_subsystem_req_set_subsystem(payload,
						      FU_MM_QCDM_DEVICE_SUBSYSTEM_OPERATIONS);
	fu_mm_qcdm_device_subsystem_req_set_subsystem_command(
	    payload,
	    FU_MM_QCDM_DEVICE_SUBSYSTEM_COMMAND_REBOOT_EDL);

	/* initialize qcdm packet data with frame and crc */
	pkt_req = fu_mm_qcdm_device_pkt_reboot_edl_req_res_new();
	fu_mm_qcdm_device_pkt_reboot_edl_req_res_set_crc(
	    pkt_req,
	    fu_crc16(FU_CRC_KIND_B16_ISO_HDLC, payload->data, payload->len));
	if (!fu_mm_qcdm_device_pkt_reboot_edl_req_res_set_payload(pkt_req, payload, error))
		return FALSE;
	...
	pkt_res = fu_mm_qcdm_device_pkt_reboot_edl_req_res_parse(g_bytes_get_data(res, NULL),
...
{
	if (!fu_mm_qcdm_device_switch_to_edl(device, error))
		return FALSE;
