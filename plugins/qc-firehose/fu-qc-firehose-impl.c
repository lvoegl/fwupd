				   ...
				   GError **error)
{
	...
	/* load patch XML */
	blob_patch = fu_firmware_get_image_by_id_bytes(firmware, fnglob_patch, &inner_error);
	if (blob_patch == NULL) {
		/* continue even without patch XML */
		g_info("failed to find patch file %s: %s", fnglob_patch, inner_error->message);
	} else {
		if (!xb_builder_source_load_bytes(source_patch, blob_patch, XB_BUILDER_SOURCE_FLAG_NONE, error)) {
